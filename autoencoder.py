...
# units, dropout, max_samples, learning_rate, batch, embedding_size (0 = onehot), lookback,
...
sims = [[128, 0.2, -1, 0.001, 512, 0, 5, True, [], 1]]
        #[128, 0.2, -1, 0.001, 512, 0, 10, True, [], 1]],
        #[128, 0.2, -1, 0.001, 512, 0, 15, True, [], 1]],
        #[128, 0.2, -1, 0.001, 512, 25, 15, True, [], 1]],
        #[128, 0.2, -1, 0.001, 512, 100, 15, True, [], 1]]]
...
for sim_ind, comb in enumerate(sims):
    ...
    name = 'AUTOENCODER ' + str(inp[0]) + ', ' + inp[2] + ', ' + inp[3] + ' (' + str(inp[4]) + ' x ' + \
    ...
    for i, df in enumerate(df_list):
        ...
        for j in range(lookback, narray.shape[0]-lookforward-output_seq+2):
            ...
            cursor += 1
    utils.plot_turbine_ids(target[:, 0], 'Total data', folder='plots')
    ...
    utils.plot_turbine_ids(train_target[:, 0], 'Train data', folder='plots')
    utils.plot_turbine_ids(val_target[:, 0], 'Validation data', folder='plots')
    utils.plot_turbine_ids(test_target[:, 0], 'Test data', folder='plots')
    ...
    confusion_matrix = np.zeros([output_seq, nclasses, nclasses])
    y_true = np.argmax(test_target, axis=-1)
    y_true = y_true.astype(int)
    for j in range(y_pred.shape[1]):
        for i in range(y_pred.shape[0]):
            confusion_matrix[j, y_pred[i, j], y_true[i, j]] += 1
        confusion_matrix = confusion_matrix.astype(int)
        id_acc, id_prec = utils.plot_confusion_matrix(confusion_matrix[j], name + str(j))
